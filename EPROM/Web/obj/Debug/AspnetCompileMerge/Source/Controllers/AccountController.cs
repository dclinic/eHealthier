    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe))
            {
                ...
                {
                    WebSecurity.Logout();
                    ModelState.AddModelError("", "The user name or password is not match for admin.");
                }
            }
            else
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    if (!WebSecurity.UserExists(model.Email))
                    {
                        ...
                        //Roles.AddUserToRole(model.Email, "provider");
        ...
        public ActionResult ExternalLoginConfirmation(RegisterExternalLoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    ePRom.Models.UserProfile user = db.UserProfiles.FirstOrDefault(u => u.UserName.ToLower() == model.UserName.ToLower());
                    // Check if user already exists
                    if (user == null)
                    {
                        // Insert name into the profile table
                        db.UserProfiles.Add(new ePRom.Models.UserProfile { UserName = model.UserName });
                        db.SaveChanges();
        ...
        public ActionResult ForgotPassword(ForgotPasswordModel model, bool captchaValid, string captchaErrorMessage)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    if (captchaValid)
                    {
                        ...
                        if (UserInAdmin.Length > 0)
                        {
                            ...
                            {
                                string URL = Request.Url.Scheme + "://" + Request.Url.Authority + "/Account/ValidateToken";
                                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(Request.Url.Scheme + "://" + Request.Url.Authority + "/api/Email/ForgotPasswordEmail?ToAddress=" + Email + "&URL=" + URL);
                                request.Method = "GET";
                                ...
                                if (result == "1")
                                {
                                    ModelState.AddModelError("", "Forgot password link to send successfully to your email address.");
                                }
                                else
                                {
                                    ModelState.AddModelError("", "There is some isuues. Please try again.");
                                }
                            }
                        }
                        else
                        {
                            ModelState.AddModelError("", "Please enter admin user name only");
                        }
                    }
                    else
                    {
        ...
        public ActionResult ValidateToken(string Token)
        {
            try
            {
                ...
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(Request.Url.Scheme + "://" + Request.Url.Authority + "/api/Email/ValidateToken?Token=" + Token);
                request.Method = "GET";
                request.ContentType = "application/json";
                WebResponse response = request.GetResponse();
                string result = new StreamReader(response.GetResponseStream()).ReadToEnd();
                ...
                if (!string.IsNullOrEmpty(result))
                {
                    ...
                    if (objArray.Length > 0)
                    {
                        if (objArray[0] == "1")
                        {
                            ...
                            return Redirect("/Account/ResetPassword?Token=" + Email);
                        }
                    }
                }
            }
