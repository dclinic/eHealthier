    ...
    $scope.PatientEprom = {
        ...
        Methods: {
            Initialize: function () {
                ...
                if ($window.localStorage.getItem("ProviderID") != null && $window.localStorage.getItem("ProviderID") != "" && $window.localStorage.getItem("PatientId") != undefined && $window.localStorage.getItem("PatientId") != null && $window.localStorage.getItem("PatientId") != "") {
                    $scope.PatientEprom.Services.getPatientEpromList($scope.PatientEprom.ProviderId, $scope.PatientEprom.Item.PatientID);
                }
                ...
                $scope.PatientEprom.Methods.setPatientAvailableEprom();
            ...
            CreatePatientSurvey: function (item, isReassign) {
            ...
            CreatePatientSurveyList: function () {
                ...
                $scope.PatientEprom.Services.CreatePatientSurveyList($scope.PatientEprom.ProviderId, $scope.PatientEprom.Item.PatientID, ids);

                window.location.href = "/PatientEprom/Index";
            },

            setPatientAvailableEprom() {
                $scope.PatientEprom.Services.getPatientAvailableEprom($scope.PatientEprom.ProviderId, $scope.PatientEprom.Item.PatientID);
            },
            ...
            SetSurveyListByPatientId: function (data) {
                ...
                $scope.PatientEprom.Item.surveyList = JSON.parse(data);

                if ($scope.PatientEprom.list_Survey != null) {
                    if ($scope.PatientEprom.list_Survey.length == 0)
                        $scope.PatientEprom.list_Survey = JSON.parse(data);

                    if (window.location.pathname.toLowerCase().indexOf("patienteprom/index") > -1) {
                        for (var i = 0; i < $scope.PatientEprom.Item.surveyList.length; i++) {
                            for (var j = 0; j < $scope.PatientEprom.list_Survey.length; j++) {
                                if ($scope.PatientEprom.Item.surveyList[i].SurveyID == $scope.PatientEprom.list_Survey[j].SurveyID) {
                                    $scope.PatientEprom.list_Survey.splice(j, 1);
                                }
                            }
                        }
                    }
                ...
                if ($scope.PatientEprom.tmp_Survey != null) {
                    for (var j = 0; j < $scope.PatientEprom.tmp_Survey.length; j++) {
                        for (var i = 0; i < $scope.PatientEprom.Item.surveyList.length; i++) {
                            if ($scope.PatientEprom.Item.surveyList[i].SurveyID == $scope.PatientEprom.tmp_Survey[j].SurveyID) {
                                $scope.PatientEprom.tmp_Survey.splice(j, 1);
                            }
                        }
                    }

                    for (var i = 0; i < $scope.PatientEprom.tmp_Survey.length; i++) {
                        var date = new Date();
                        var newdate = new Date(date);
                        newdate.setDate(newdate.getDate() + Number($scope.PatientEprom.tmp_Survey[i].Days));
                        var res = new Date(newdate);

                        var newRow = {
                            SurveyID: $scope.PatientEprom.tmp_Survey[i].SurveyID,
                            StartDate: new Date(),
                            EndDate: res,
                            IsActive: true,
                            isnewRow: true,
            ...
            SetSurveyMonkeyResponseResult: function (response) {
                ...
                for (var i = 0; i < $scope.PatientEprom.AnswerResult.data.length; i++) {
                    ...
                    console.log($scope.PatientEprom.AnswerResult.data[i])
        ...
        Services: {
            ...
            CreatePatientSurvey: function (data) {
                $http.post('/api/eproms/AddPatientSurvey/', data).success(function (response) {
                    if (response != "") {
                        ...
                        $scope.PatientEprom.Methods.GetSurveyByPatient_Provider_Org_Practice();
                        $scope.PatientEprom.IsLoading = false;
                        toaster.pop('success', 'Success', "Eprom assigned successfully.");
                        $scope.PatientEprom.issubmitted = false;
                    }
                    else {
                        $scope.PatientEprom.IsLoading = false;
            ...
            CreatePatientSurveyList: function (providerID, patientID, surveyID) {
                $http.post('/api/Eproms/AddPatientSurveyEProms?providerID=' + providerID + '&patientID=' + patientID + '&surveyID=' + surveyID).success(function (response) { });
            },

            getPatientEpromList: function (providerID, patientID) {
                $http.get('/api/Eproms/GetPatientSurveyEProms?providerID=' + providerID + '&PatientId=' + patientID).success(function (response) {
                    $scope.PatientEprom.tmp_Survey = JSON.parse(response);
                });
            },

            getPatientAvailableEprom: function (providerID, patientID) {
                $http.get('/api/Eproms/GetPatientSurveyEProms?providerID=' + providerID + '&PatientId=' + patientID).success(function (response) {
                    $scope.PatientEprom.list_Survey = JSON.parse(response);
                });
            },
