app.controller('patientController', ['$scope', '$http', '$window', '$location', '$upload', 'toaster', '$ngBootbox', '$cookies', '$filter', function ($scope, $http, $window, $location, $upload, toaster, $ngBootbox, $cookies, $filter) {
    ...
    $scope.Patient = {
        ...
        Methods: {
            Initialize: function () {
                ...
                setTimeout(function () {
                    ...
                    if (window.location.pathname.indexOf("SearchPatient") == -1) {
            ...
            SetPatientDetails: function (data) {
                ...
                $scope.Patient.isNoData = false;

                if ($scope.Patient.Items.length == 0) {
                    $scope.Patient.isItems = true;
                }
            ...
            getPatientsName: function () {
                if ($scope.Patient.patientname.length > 2) {
                    $scope.Patient.IsLoading = true;
                    $scope.Patient.Services.getPatientsName($scope.Patient.Item.User.UserName, $scope.Patient.Item.OrganizationID, $scope.Patient.Item.PracticeID, $scope.Patient.patientname);
                }

                if ($scope.Patient.patientname.length == 0) {
                    $scope.Patient.isLoadOneTime = false;
                    $scope.Patient.Methods.GetPatientforDashboard();
                }
            },
            ...
            SearchPatientDetail: function (IHINumber, MedicareNumber) {
                $scope.Patient.issubmitted = true;
                if ($scope.frmSearchPatient.$valid) {
        ...
        Services: {
            ...
            getPatientsName: function (UserName, OrganizationId, PracticeId, patientName) {
                $http.get('/api/Patient/GetProviderPatientDetailBySeletedPatient?UserName=' + UserName + '&OrganizationId=' + OrganizationId + '&PracticeId=' + PracticeId + '&patientName=' + patientName).success($scope.Patient.Methods.SetPatientsNameList);
            },
            Register: function (data) {
